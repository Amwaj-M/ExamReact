import React from 'react'
import axios from 'axios';
import { useState , useEffect} from 'react';
import { Link } from 'react-router-dom';
import { useNavigate } from 'react-router-dom';


export default function SignIn() {

    const navigate = useNavigate();

    const [email, setEmail] = useState('')
    const [password, setPass] = useState('')

    const [user, setUser] = useState([])

    useEffect(() => {
        getData()
      }, [])
      
      function getData() {
  
          axios.get('https://fakestoreapi.com/users')
          .then(function (response) {
            // handle success
            console.log(response.data);
            setUser(response.data);
  
          })
          .catch(function (error) {
            // handle error
            console.log(error);
          })
          
      }





function handelClick(e) {
    e.preventDefault();

    const userFind = user.find(user=> user.email === email && user.pass === password)
   if (userFind){
    setUser(userFind)
    navigate('/');

   } else {
    alert('the data is not correct')
   }
    
}


  return (
    <div className='w-full min-h-screen flex justify-center items-center flex-col p-11 max-sm:p-3'>
        <div  className='w-full min-h-screen flex items-center flex-col ' >
            <img className='h-11 max-sm:mt-2' src='data:image/png;base64,'/>
        <div className='w-[30%] border border-[#0000008d] mt-3 rounded-md flex flex-col gap-2 px-5 max-sm:w-full'>
            <h1 className='font-medium text-2xl pt-5'>Sign in</h1>
            <h1 className='font-medium'>Email</h1>
            <input type='text' className='border border-black rounded-md px-1 py-2' value={email}
            onChange={(e)=> setEmail(e.target.value)}
            />

            <h1 className='font-medium'>Password</h1>
            <input type='password' className='border border-black rounded-md px-1 py-2'
             value={password}
             onChange={(e)=> setPass(e.target.value)} />

        <button onClick={handelClick} className='bg-[#ffd814ff] rounded-md hover:bg-[#ffd814] py-2 mt-3'>Sign In</button>

        <h1 className='font-normal'>
        By continuing, you agree to Amazon's <a href='https://www.amazon.com/gp/help/customer/display.html/ref=ap_signin_notification_condition_of_use?ie=UTF8&nodeId=508088' className='text-[#0066bfff]'>Conditions of Use</a>
          and <a href='https://www.amazon.com/gp/help/customer/display.html/ref=ap_signin_notification_privacy_notice?ie=UTF8&nodeId=468496'className='text-[#0066bfff]'> Privacy Notice. </a>
        </h1>

        <h1 className='text-[#0066bfff] mt-5 pb-5'>
        Need help?
        <br></br>
        Forgot your password?
        <br></br>
        Other issues with Sign-In
        </h1>

        <h1 className='font-medium pt-2' > Buying for work?</h1>
        <h1 className='text-[#0066bfff] pb-3'>Shop on Amazon Business</h1>


        </div>

        </div>
        
    </div>
  )
}
